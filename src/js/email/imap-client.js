...
// Note: this was originally the separate npm package imap-client.
// I pulled it into the source tree so I could update it to use
// emailjs-imap-client instead of browserbox.

var Imap = require('emailjs-imap-client'),
...
ImapClient.prototype.login = function() {
    ...
    if (self._loggedIn) {
        ...
        return self._client.connect().then(function() {
...
ImapClient.prototype.search = function(options) {
    ...
    return self._checkOnline().then(function() {
        return client.search(options.path, query, queryOptions);
    }).then(function(uids) {
...
ImapClient.prototype.updateFlags = function(options) {
    ...
    return self._checkOnline().then(function() {
        return new Promise(function(resolve) {
            if (add.length > 0) {
                resolve(self._client.setFlags(options.path, interval, queryAdd, queryOptions));
            } else {
                resolve();
            }
        });
        ...
        if (remove.length > 0) {
            return self._client.setFlags(options.path, interval, queryRemove, queryOptions);
        }
    }).then(function() {
...
ImapClient.prototype.moveMessage = function(options) {
    ...
    return self._checkOnline().then(function() {
        return self._client.moveMessages(options.path, interval, options.destination, queryOptions);
    }).then(function() {
...
ImapClient.prototype.deleteMessage = function(options) {
    ...
    return self._checkOnline().then(function() {
        return self._client.deleteMessages(options.path, interval, queryOptions);
    }).then(function() {
